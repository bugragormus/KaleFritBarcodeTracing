                ...
                <div class="card card-modern">
                    <div class="card-header card-header-modern">
                        <h3 class="card-title-modern mb-0">
                            <i class="fas fa-list"></i> ÜRETİM LİSTESİ (PALETLER)
                        </h3>
                    </div>

                    <div class="card-body card-body-modern p-0">
                        <div class="table-responsive">
                            <table class="table table-hover yajra-datatable w-100 m-0">
                                <thead style="background: #f8f9fa;">
                                    <tr>
                                        <th class="pl-4">PALET NO</th>
                                        <th>FRİT KODU</th>
                                        <th>ŞARJ NO</th>
                                        <th>BOYUT</th>
                                        <th>KULL. MİKTAR</th>
                                        ...
                                        <th>ÜRETİM TARİHİ</th>
                                        <th class="text-center pr-4">İŞLEMLER</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="pl-4">
                                            <a href="{{ route('granilya.production.show', $prod->pallet_number) }}">
                                                <strong>{{ $prod->pallet_number }}</strong>
                                            </a>
                                        </td>
                                        <td>{{ $prod->stock ? $prod->stock->name : '-' }}</td>
                                        <td>{{ $prod->load_number }}</td>
                                        <td>{{ $prod->size ? $prod->size->name : '-' }}</td>
                                        <td>{{ $prod->used_quantity }} KG</td>
                                        <td>{{ $prod->user ? $prod->user->name : '-' }}</td>
                                        <td>
                                            @if($prod->general_note)
                                                <span title="{{ $prod->general_note }}">{{ Str::limit($prod->general_note, 20) }}</span>
                                            @else
                                                -
                                            @endif
                                        </td>
                                        <td class="text-center">{!! $prod->status_badge !!}</td>
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $('.yajra-datatable').DataTable({
                ...
                order: [[ 8, "desc" ]], // Üretim Tarihi descending
                pageLength: 25,
            ...
            fields.forEach(field => {
                ...
                if(values.length > 0) {
                    ...
                    if(val) {
                        $('#' + field + 'Filter').val(val).trigger('change');
                    }
                }
            });

            // Optional: 'Enter' key on text inputs clicks apply
            ...
            multiFields.forEach(field => {
                ...
                if(selections && selections.length > 0) {
                    if(Array.isArray(selections)) {
                        ...
                        params.append(field, selections);
                    }
                }
            });
