...
<div class="page-title-box d-flex align-items-center justify-content-between">
    <div>
        <h4><i class="fas fa-layer-group mr-2" style="opacity:.9;"></i> Granilya Siparişleri</h4>
        <p>Toplam satışa hazır Granilya stoku: <strong>{{ number_format($totalGranilyaStock, 0, ',', '.') }} KG</strong></p>
    </div>
...
<div class="card card-modern mb-4">
    <div class="card-body py-3 px-4">
        <form method="GET" action="{{ route('orders.granilya.index') }}" class="row align-items-end">
            ...
            <div class="col-md-3 d-flex gap-2">
                <button type="submit" class="btn btn-success btn-sm" style="border-radius:7px;font-weight:600;"><i class="fas fa-search mr-1"></i> Filtrele</button>
                <a href="{{ route('orders.granilya.index') }}" class="btn btn-light btn-sm ml-2" style="border-radius:7px;">Temizle</a>
            </div>
        </form>
    </div>
</div>
...
<div class="card card-modern">
    <div class="card-body text-center py-5">
        <div style="width:72px;height:72px;border-radius:50%;background:#d1fae5;display:flex;align-items:center;justify-content:center;margin:0 auto 1rem;">
            <i class="fas fa-layer-group" style="font-size:30px;color:#059669;"></i>
        </div>
        <h5 class="text-muted">{{ request()->hasAny(['company_id','status','date_from','date_to']) ? 'Filtreye uygun sipariş bulunamadı.' : 'Henüz granilya siparişi yok.' }}</h5>
        <a href="{{ route('orders.granilya.create') }}" class="btn btn-success mt-3" style="border-radius:9px;">Yeni Sipariş Ekle</a>
    </div>
</div>
@else
<div class="card card-modern">
    <div class="card-header d-flex align-items-center justify-content-between">
        <span><i class="fas fa-list mr-2" style="color:#059669;"></i> Sipariş Listesi</span>
        <span class="badge" style="background:#d1fae5;color:#065f46;border-radius:20px;padding:4px 12px;">{{ $orders->count() }} adet</span>
    </div>
    <div class="card-body p-0">
        <div class="table-responsive">
            <table class="table table-hover mb-0" id="granilyaOrdersTable">
                ...
                <tbody>
                    @foreach($orders as $order)
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge" style="background:#d1fae5;color:#065f46;border-radius:6px;padding:4px 10px;">
                            ...
                            <td>
                                @if($a['is_sufficient'])
                                    <span class="badge badge-sufficient px-2 py-1" style="border-radius:6px;font-size:11px;">✅ Yeterli</span>
                                @else
                            ...
                            <td class="text-center">
                                <a href="{{ route('orders.granilya.show', $order) }}" class="btn btn-sm btn-light" style="border-radius:6px;" title="Detay / Düzenleme">
                                    <i class="fas fa-edit" style="color:#059669;"></i>
                                </a>
                                @if($order->notes)
                                ...
                                @endif
                                <button class="btn btn-sm btn-light btn-delete-order ml-1" data-id="{{ $order->id }}" data-url="{{ route('orders.granilya.destroy', $order) }}" style="border-radius:6px;">
...
$(document).ready(function () {
    if ($('#granilyaOrdersTable').length) {
        $('#granilyaOrdersTable').DataTable({
